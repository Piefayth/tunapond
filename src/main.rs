...
use service::payment::{payout_updater, payment_updater, payment_verifier};
...
async fn main() -> std::io::Result<()> {
    ...
    tokio::spawn(payout_updater(pool.clone()));
    tokio::spawn(payment_updater(pool.clone()));
    tokio::spawn(payment_verifier(pool.clone()));
